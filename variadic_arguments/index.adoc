...
----
template<typename... Ts>
const void* get(const std::tuple<Ts...>& tup, size_t index);
...
const void* get(const std::tuple<Ts...>& tup, size_t index) {
    switch (index) {
        case 0: return &std::get<0>(tup);
        case 1: return &std::get<1>(tup);
        case 2: return &std::get<2>(tup);
...
const void* get(const std::tuple<Ts...>& tup, size_t index) {
    auto getE = [&]<size_t I>(std::in_place_index_t<I>, size_t index) -> const void* {
        return I == index ? &std::get<I>(tup) : nullptr;
    };
