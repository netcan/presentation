...
struct dumpImpl {
    ...
    void operator()(const Tup& tp) const {
        dumpImpl<N-1>{} (tp);
        std::cout << std::get<N>(tp) << std::endl;
    }
};
...
void dumpImpl(const Tup& tp, std::index_sequence<I...>) {
    auto l = { ((std::cout << std::get<I>(tp) << std::endl), 0)... };
}
...
C++20 折叠表达式 + apply + 模板lambda
[source,cpp]
----
jemplate<typename... Ts>
void dump(const std::tuple<Ts...>& tp) {
    std::apply([]<typename... Args>(const Args&... args) {
